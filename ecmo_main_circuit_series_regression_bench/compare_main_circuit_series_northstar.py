...
subprocess.run(
    [
        ...
        str(
            ...
            / "run_main_circuit_series_northstar.py"
        ),
    ],
    check=True,
)
a = json.loads(
    (
        ROOT
        / "ecmo_main_circuit_series_regression_bench"
        / "accepted_ecmo_main_circuit_series_northstar_v1.json"
    ).read_text()
)
c = json.loads(
    (
...
if fail:
    print("ECMO Main Circuit Series NorthStar differences outside tolerance:")
    for x in fail:
        print(x)
    raise SystemExit(1)
print("ECMO Main Circuit Series NorthStar v1: PASS — 0 differences outside tolerance")
