...
out = {"schema": "ecmo-main-circuit-with-shunt-and-bridge-northstar-v1", "cases": {}}

# Case 1: RPM sweep at bridge closed (regression check vs Stage 2)
rpm_sweep = {}
for rpm in RPM_STEPS:
    point = solve_main_circuit_with_shunt_and_bridge_operating_point(float(rpm))
...
# Case 2: clamp-position sweep at fixed RPM (weaning-trial behavior)
clamp_sweep = {}
for clamp in CLAMP_STEPS:
    point = solve_main_circuit_with_shunt_and_bridge_operating_point(
...
path = (
    ROOT
    / "ecmo_main_circuit_with_shunt_and_bridge_regression_bench"
    / "current_ecmo_main_circuit_with_shunt_and_bridge_northstar.json"
)
