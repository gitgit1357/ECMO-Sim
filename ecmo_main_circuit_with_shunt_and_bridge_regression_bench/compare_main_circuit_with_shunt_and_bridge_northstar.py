...
subprocess.run(
    [
        ...
        str(
            ...
            / "run_main_circuit_with_shunt_and_bridge_northstar.py"
        ),
    ],
    check=True,
)
a = json.loads(
    (
        ROOT
        / "ecmo_main_circuit_with_shunt_and_bridge_regression_bench"
        / "accepted_ecmo_main_circuit_with_shunt_and_bridge_northstar_v1.json"
    ).read_text()
)
c = json.loads(
    (
...
if fail:
    print("ECMO Main Circuit + Shunt + Bridge NorthStar differences outside tolerance:")
    for x in fail:
        print(x)
    raise SystemExit(1)
print("ECMO Main Circuit + Shunt + Bridge NorthStar v1: PASS — 0 differences outside tolerance")
